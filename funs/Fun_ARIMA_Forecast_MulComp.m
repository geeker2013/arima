...
if ~isempty(strfind(version,'2018'))||~isempty(strfind(version,'2017'))||~isempty(strfind(version,'2016')) %依据当前MATLAB版本执行不同语句
    [forData,YMSE] = forecast(EstMdl,testLen,'Y0',data);   %matlab2018及以下版本写为Predict_Y = forecast(EstMdl,step,'Y0',Y);   matlab2019写为Predict_Y = forecast(EstMdl,step,Y);
elseif ~isempty(strfind(version,'2019'))||~isempty(strfind(version,'2020'))||~isempty(strfind(version,'2021'))
    [forData,YMSE] = forecast(EstMdl,testLen,data);   %matlab2018及以下版本写为Predict_Y = forecast(EstMdl,step,'Y0',Y);   matlab2019写为Predict_Y = forecast(EstMdl,step,Y);
else
...
hold on
h4 = plot(length(data):length(data)+testLen,[data(end);lower],'r:','LineWidth',2);   %绘制置信区间下限
plot(length(data):length(data)+testLen,[data(end);upper],'r:','LineWidth',2)         %绘制置信区间上限
...
h3 = plot(TrainLen:totalLen,[data(end);forData],'r');  %绘制预测曲线
legend([h1 h2 h3 h4],'训练集数据','测试集真实值','测试集预测值','95% 置信区间',...  %图例
	     'Location','NorthWest')
title('多步预测效果对比')
hold off
end
